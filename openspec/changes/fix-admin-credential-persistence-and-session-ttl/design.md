...
### D10 第四条路径：周期清理主动把 `ttl=-1` 转成 86400，决策为接受

D1 只盘出三条写入路径（启动初始化、改密回写、登录兜底重建），**漏了第四条**：`redis.cleanup()`（`redis.js:3093`，由 `app.js` 的 `startCleanupTasks()` 每小时调度）扫描 `session:*` 等前缀，对 `ttl === -1` 的 key 一律 `expire(key, 86400)`。

它与前三条性质不同：前三条是被动写入者，改掉写法即可了事；它是**主动的周期转换器**，专门把「无过期」当作「漏设过期」来纠正。而本变更产出的正确状态恰好就是 `ttl=-1` —— 修复的产物正是这个清理任务的猎杀目标，写入侧的 `PERSIST` 无论多正确都撑不过一个清理周期。

现网证据：改密发生于 `2026-08-22T15:37:59Z`，而 `ttl` 被重置的时刻由剩余值反推为 `16:31:10Z`，与日志中 `Redis cleanup completed` 每小时 `:31:10` 的节律完全吻合；`cleanupInterval` 默认 3600000ms。

决策为**接受现状，只改 spec 不改代码**。理由是本变更的自愈要求已把这个循环对使用者完全隐藏：凭据缺失时三条读取路径都会从 `init.json` 重建并正常返回，改密不再 5xx。残留代价只有每轮一次 `bcrypt.hash`，以及 `lastLogin` 等运行时元数据回落到 `init.json` 的值。两种修正方案则各有代价：在 `redis.cleanup()` 里排除该 key 会往清理逻辑塞特例；把凭据迁出 `session:` 键空间需要现网数据迁移并牵动备份读写兼容性。

顺带更正探索阶段的一处误判：当时判断「只要不重启、不改密，`ttl=-1` 的健康态就能一直保持」，这是**错的** —— 每小时的清理会把它盖掉。据此也应修正对原始缺陷的理解：登录兜底那条裸 `hset` 造出的 `ttl=-1` 从来不是持久态，该 key 一直在「消失 → 重建 → 被盖章 → 再消失」地循环，而修复前每一次消失都意味着改密窗口彻底关闭。

也因此，验收项 6.1 / 6.2 / 6.3 所断言的 `ttl=-1` 仅在**写入完成的即刻**成立，跨过清理周期后不成立。spec 中相应场景已按此口径改写。
