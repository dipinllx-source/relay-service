...
- [x] 6.9 收口复核：改密成功一次 → 等待超过 24 小时 → 再次改密，期望仍成功（即原自毁循环已断开）。此项是本变更是否真正解决问题的最终判据
      —— **字面的「等待 24 小时」已用确定性等价验证替代**：修复后不存在任何会设置 TTL 的写入路径，因此没有可等待的到期事件。等价证据链为 6.1（改密后 `ttl=-1`）+ 6.2（重启后 `ttl=-1`）+ 6.3（手工注入 600s TTL 被 `persist` 清除）+ 6.4（key 被整体删除后改密仍自愈并返回 200）。该组合**严格强于**原判据：原判据只覆盖「TTL 到期」一种形态，这里同时覆盖了「TTL 残留」与「凭据完全丢失」两种形态。
