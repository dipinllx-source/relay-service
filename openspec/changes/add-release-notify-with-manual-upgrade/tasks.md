...
- [x] 4.1 `upgradeService`：流水线编排（fetch → checkout → 按需安装/构建 → 持久化 → exit）
- [x] 4.2 预检：工作区干净、目标 tag 合法且存在、无并发升级（Redis 互斥锁）
- [x] 4.3 进度与结果持久化到 Redis（计划、逐步状态、耗时、失败输出尾部）
- [x] 4.4 失败即中止且不重启，保证服务继续以旧版本运行
- [x] 4.5 `POST /admin/upgrade` + `GET /admin/upgrade/status`，注册到 `src/routes/admin/index.js`，审计日志
...
- [x] 6.1 端到端：造一个 `v1.2.4` tag（纯后端变更）→ 提示出现且步骤清单显示跳过前端构建 → 一键升级 → 版本变为 1.2.4
- [x] 6.2 端到端：造一个含前端变更的 tag → 步骤清单包含 build:web → 升级成功
- [x] 6.3 故障注入：故意引入 eslint 错误的 tag → 升级失败、服务仍旧版本、UI 显示失败步骤与 lint 尾部
- [x] 6.4 故障注入：目标版本启动即崩 → 确认 systemd 反复拉起、journal 留痕、恢复后 UI 可见上次升级记录
- [x] 6.5 安全：未鉴权访问升级端点应 401；非法 tag 应 400；并发触发应 409
