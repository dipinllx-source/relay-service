...
- [ ] 4.6 与 `manage.js update` 的关系收敛（避免两份编排实现漂移）

## 5. 管理台提示（L3）

- [ ] 5.1 恢复「检查更新」入口
- [ ] 5.2 `hasUpdate` 时渲染：版本对比、变更清单（分组）、影响面、将执行的步骤清单（含重启中断会话警示）
- [ ] 5.3 升级中/后反馈：轮询状态、跨重启续查、以 `check-updates.current` 等于目标版本判定完成
- [ ] 5.4 失败展示：失败步骤 + 输出尾部 + 明确「服务仍运行旧版本」
- [ ] 5.5 前端 API 封装与构建（`vite build`；注意 eslint 阻断，先 `eslint --fix`）
...
- [ ] 6.4 故障注入：目标版本启动即崩 → 确认 systemd 反复拉起、journal 留痕、恢复后 UI 可见上次升级记录
